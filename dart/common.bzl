...
def make_dart_context(label,
  ...
  data = set(data or [])
  deps = deps or []
  transitive_srcs, transitive_data, transitive_deps = collect_files(srcs, data, deps)
  return struct(
      label=label,
      package=package,
      lib_root=lib_root,
...
def collect_files(srcs, data, deps):
  transitive_srcs = set()
  transitive_data = set()
  transitive_deps = {}
  for dep in deps:
...
def package_spec_action(ctx, dart_ctx, output):
  ...
  for dc in dart_ctxs:
    relative_lib_root = relative_path(dart_ctx.label.package, dc.lib_root)
...
def layout_action(ctx, srcs, output_dir):
  ...
  for src_file in srcs:
    dest_file = ctx.new_file(output_dir + src_file.short_path)
