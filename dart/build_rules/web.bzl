...
def dart2js_action(
    ...
    if enable_asserts:
        dart2js_args.append("--enable-asserts")
    if csp:
        dart2js_args.append("--csp")
    if dump_info:
        dart2js_args.append("--dump-info")
    if minify:
        dart2js_args.append("--minify")
    if preserve_uris:
        dart2js_args.append("--preserve-uris")
    dart2js_args.append(out_script.path)
    ctx.actions.run(
        inputs = inputs,
        tools = tools,
...
def _dart_web_application_impl(ctx):
    ...
    if ctx.attr.dump_info:
        other_outputs.append(ctx.outputs.info_json)
    part_outputs = []
    for i in range(1, ctx.attr.deferred_lib_count + 1):
        part_outputs.append(getattr(ctx.outputs, "part_js%s" % i))
        other_outputs.append(getattr(ctx.outputs, "part_sourcemap%s" % i))
