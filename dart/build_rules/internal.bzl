...
a mechanism for enforcing limitied visibility of Starlark rules. This code makes
...
def assert_third_party_licenses(ctx):
    ...
    if (not ctx.attr.testonly and
        not ctx.attr.license_files and
        ctx.label.package.startswith(_third_party_prefix)):
...
def package_spec_action(ctx, dart_ctx, output):
    ...
    dart_ctxs = _collect_dart_context(
        dart_ctx,
        transitive = True,
        include_self = True,
    ).values()
...
def layout_action(ctx, srcs, output_dir):
    ...
    for src_file in srcs:
        ...
        commands.append("ln -s '%s' '%s'" % (link_target, dest_file.path))
